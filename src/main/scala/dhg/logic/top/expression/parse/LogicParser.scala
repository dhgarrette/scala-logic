package dhg.logic.top.expression.parse
    ...
    extends ParseException(Some(index),
        unexpected match {
            case Some(unexpected) => {
                if (expected.isEmpty)
                    String.format("Unexpected token: '%s'.  %s", unexpected, message.getOrElse(""))
                else if (expected.length == 1)
                    String.format("Unexpected token: '%s'.  Expected token '%s'.", unexpected, expected.head)
                else
                    String.format("Unexpected token: '%s'.  Expected one of %s.", unexpected, expected.mkString("[", ", ", "]"))
            }
            case None => String.format("Expected token '%s'.", expected)
        }, nested)
