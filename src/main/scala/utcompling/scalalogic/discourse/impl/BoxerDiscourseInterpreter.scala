...
class BoxerDiscourseInterpreter[T](
    ...
    private def parseBoxerOutput(boxerOut: String): Map[String, Option[T]] = {
        ...
        val re = """id\((\S+),(\d+)\)\.\s*sem\((\d+),\s*\[\S*\],\s*\[\S*\],\s*\[\S*\],\s*(\S*)\s*\)\.""".r
        for (m <- re.findAllIn(boxerOut).matchData) {
            val List(discourseId, drsId, drsId2, drsInput) = m.subgroups
            require(drsId == drsId2)
